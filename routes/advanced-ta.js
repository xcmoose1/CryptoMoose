...
const { RSI, MACD, BollingerBands } = pkg;
...
router.get('/mtf/:symbol', async (req, res) => {
    try {
        ...
        for (const timeframe of timeframes) {
            const response = await axios.get(`https://api.mexc.com/api/v3/klines?symbol=${symbol}&interval=${timeframe}&limit=100`);
            mtfData[timeframe] = response.data;
        }
...
router.get('/analysis/:symbol/:timeframe', async (req, res) => {
    try {
        ...
        const analysis = {
            ...
            divergences: findDivergences(data)
        };
