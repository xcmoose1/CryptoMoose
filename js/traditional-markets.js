...
class TraditionalMarketsHandler {
    ...
    async fetchIndicesData() {
        for (const [key, symbol] of Object.entries(this.symbols.indices)) {
            try {
                const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?range=${this.timeframe}&interval=5m`);
    ...
    async fetchCommoditiesData() {
        for (const [key, symbol] of Object.entries(this.symbols.commodities)) {
            try {
                const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?range=${this.timeframe}&interval=5m`);
                const data = await response.json();
